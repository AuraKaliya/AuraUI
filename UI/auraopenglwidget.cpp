...
AuraOpenGLWidget::AuraOpenGLWidget(QWidget *parent)
    : QOpenGLWidget{parent}
{
    ...
    m_baseDir=new QDir(qApp->applicationDirPath()+"/OpenGLSource");
    m_modelPath=m_baseDir->path()+"/model";
    m_shaderPath=m_baseDir->path()+"/shaderProgram";
...
void AuraOpenGLWidget::check(AuraOpenGLObject *obj)
{
    qDebug()<<"===================";
    qDebug()<<"==========posList============";
...
void AuraOpenGLWidget::initObject(AuraOpenGLObject *obj)
{
...
void AuraOpenGLWidget::reInitObject(AuraOpenGLObject *obj)
{
...
double AuraOpenGLWidget::posXFromOpenGLToQt(double posX)
{
    return ((posX+1.0)/2) * this->width();
}

double AuraOpenGLWidget::posYFromOpenGLToQt(double posY)
{
    //因为Qt左手坐标系 opengl右手坐标系，原点也不同，所以在这里需要进行一次统一
    return this->height()-((posY+1.0)/2) * this->height();
}

double AuraOpenGLWidget::posXFromQtToOpenGL(double posX)
{
    return (posX*2)/this->width()-1.0;
}

double AuraOpenGLWidget::posYFromQtToOpenGL(double posY)
{
    return ((this->height()-posY)*2)/this->height()+-1.0;
}

void AuraOpenGLWidget::initSetting()
{
    m_shift=false;
...
void AuraOpenGLWidget::initShaderProgram()
{
    //从文件中读入源----初始化shaderProgram
    bool success;

    m_shaderProgram->addCacheableShaderFromSourceFile(QOpenGLShader::Vertex,QString(   m_shaderPath+"/shapes.vert"));
    m_shaderProgram->addCacheableShaderFromSourceFile(QOpenGLShader::Fragment,QString( m_shaderPath+"/shapes.frag"));
    ...
    m_borderProgram->addCacheableShaderFromSourceFile(QOpenGLShader::Vertex,QString(   m_shaderPath+"/shapes.vert"));
    m_borderProgram->addCacheableShaderFromSourceFile(QOpenGLShader::Fragment,QString( m_shaderPath+"/border.frag"));
...
void AuraOpenGLWidget::initObjData()
{
    QStringList filter;
    filter<<"*.obj";
    QDir modelDir(m_modelPath);
    modelDir.setNameFilters(filter);
    for(auto it: modelDir.entryList(filter,QDir::Files | QDir::Readable,QDir::Name) )
    {
        AuraOpenGLObject* obj=new AuraOpenGLObject;
        obj->name="Cube";
        //qDebug()<<it;
        obj->normalVectorFlag=false;
        obj->objNormalFlag=false;

        QString path=modelDir.path()+"/"+it;
        QFile file(path);
...
Eigen::Vector3d AuraOpenGLWidget::getNormalVector(AuraOpenGLObject *obj, int point1, int point2, int point3)
{
...
void AuraOpenGLWidget::testNormalVector(QVector3D point1, QVector3D point2, QVector3D point3)
{
...
bool AuraOpenGLWidget::intersectTriangle(const QVector3D &origPos, const QVector3D &rayDir,
...
bool AuraOpenGLWidget::checkSelectModel(const QVector3D &origPos, const QVector3D &rayDir, AuraOpenGLObject *obj)
{
    //qDebug()<<"==================AuraOpenGLWidget::checkSelectModel======================";
    bool res=false;
    ...
    //qDebug()<<"==================AuraOpenGLWidget::checkSelectModel======================end";
    return res;
}

bool AuraOpenGLWidget::rayTriangleIntersect(const Eigen::Vector3d &orig, const Eigen::Vector3d &dir, const Eigen::Vector3d &v0, const Eigen::Vector3d &v1, const Eigen::Vector3d &v2)
{
    // 计算三角形的两个边
...
QVector3D AuraOpenGLWidget::getRayDirection(double posX, double posY, const QMatrix4x4 &rotationMatrix, const QMatrix4x4 &translateMatrix, const QVector3D &cameraPos)
{
    // 将平面坐标转换为齐次坐标
...
QVector3D AuraOpenGLWidget::getIntersectionWithZPlane(const QVector3D &origin, const QVector3D &direction, double z)
{
    if (direction.z() == 0) {
...
void AuraOpenGLWidget::initializeGL()
{
...
void AuraOpenGLWidget::resizeGL(int w, int h)
{
    if (h == 0) { // 防止被零除
        h = 1; // 将高设为1
    }
...
void AuraOpenGLWidget::paintGL()
{
   ...
   // glClearDepth(1.0f);
    ...
    //处理模型
    AuraOpenGLObject* modelObj=m_obj;
...
void AuraOpenGLWidget::keyPressEvent(QKeyEvent *e)
{
...
void AuraOpenGLWidget::keyReleaseEvent(QKeyEvent *e)
{
    m_shift=false;
    //m_alt=false;
...
void AuraOpenGLWidget::wheelEvent(QWheelEvent *e)
{
...
void AuraOpenGLWidget::mouseMoveEvent(QMouseEvent *e)
{
...
void AuraOpenGLWidget::mousePressEvent(QMouseEvent *e)
{
    m_startPoint=e->pos();
...
void AuraOpenGLWidget::mouseReleaseEvent(QMouseEvent *e)
{

}

void AuraOpenGLWidget::paintEvent(QPaintEvent *e)
{
